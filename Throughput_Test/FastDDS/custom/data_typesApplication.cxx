...
/*!
 * @file data_typesApplication.cxx
...
std::shared_ptr<data_typesApplication> data_typesApplication::make_app(
        const int& domain_id,
        const std::string& entity_kind)
{
    std::shared_ptr<data_typesApplication> entity;
    if (strcmp(entity_kind.c_str(), "publisher") == 0)
    {
        entity = std::make_shared<data_typesPublisherApp>(domain_id);
    }
    else if (strcmp(entity_kind.c_str(), "subscriber") == 0)
    {
        entity = std::make_shared<data_typesSubscriberApp>(domain_id);
    }
    else
    {
