...
latencyPublisherApp::latencyPublisherApp(const int& domain_id, int samples_to_send, int warmup_samples, std::string json_file_path)
...
void latencyPublisherApp::run() {
    ...
    for (int i = 0; i < warmup_samples_ && !is_stopped(); ++i) {
        ...
        // Wait for the bounce with a second safety timeout
    ...
    while (samples_sent < samples_to_send_ && !is_stopped()) {
        ...
        // Freeze this thread until the 'on_data_available' callback 
        std::unique_lock<std::mutex> pong_lock(pong_mutex_);
        pong_cv_.wait(pong_lock, [this]() { return pong_received_ || is_stopped(); });

        // Stop the timer immediately upon waking up
        auto end = std::chrono::high_resolution_clock::now();

        if (is_stopped()) break;

        // Calculate RTT in microseconds and divide by 2 (One-Way Latency)
        double rtt_us = std::chrono::duration<double, std::micro>(end - start).count();
        times_.push_back(rtt_us / 2.0);
...
void latencyPublisherApp::stop() { stop_.store(true); match_cv_.notify_all(); pong_cv_.notify_all(); }
