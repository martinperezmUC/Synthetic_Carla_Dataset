...
for node in "${SUB_NODES[@]}"; do
    sshpass -p "$PASS" ssh -f $SSH_OPTS -p "$PORT" "$USER@$node" \
        "cd $WORKSPACE_DIR/Latency_Test_RTT && nohup ./latency_host_runner.sh subscriber > /dev/null 2>&1 &"
done
...
sshpass -p "$PASS" ssh $SSH_OPTS -p "$PORT" "$USER@$PUB_NODE" \
    "cd $WORKSPACE_DIR/Latency_Test_RTT && ./latency_host_runner.sh publisher" | tee "$LOG_FILE" | tee "$LOG_FILE"

echo "[+] Publisher ended. Cleaning subscribers..."
for node in "${SUB_NODES[@]}"; do
    sshpass -p "$PASS" ssh $SSH_OPTS -p "$PORT" "$USER@$node" "pkill -f Latency_Test_RTT_bin"
done 
